...
def lambda_handler(event, context):
	...
	if not role or not user_id:
		...
		{
            'statusCode': 400,
            'body': json.dumps({'error': 'role and user_id are required'})
		}
    ...
    try:
        ...
        with connection.cursor() as cursor:
            sql = "UPDATE UserInformation SET role = %s WHERE user_id = %s"
            cursor.execute(sql, (role, user_id))
            connection.commit()
    except Exception as e:
        logger.error("Failed to set role: %s", str(e))
        return {
            'statusCode': 500,
            'body': json.dumps({'error': 'Failed to set role.', 'message': str(e)})
        }
    finally:
        connection.close()
    ...
    {
        'statusCode': 200,
        'body': f'Hello, {username} from CreateRole!'
    }
