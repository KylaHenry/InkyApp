...
def lambda_handler(event, context):
	...
	if not description or not user_id:
		...
		{
            'statusCode': 400,
            'body': json.dumps({'error': 'description and user_id are required'})
		}
    ...
    try:
        ...
        with connection.cursor() as cursor:
            sql = "UPDATE UserInformation SET description = %s WHERE user_id = %s"
            cursor.execute(sql, (description, user_id))
            connection.commit()
    except Exception as e:
        logger.error("Failed to set description: %s", str(e))
        return {
            'statusCode': 500,
            'body': json.dumps({'error': 'Failed to set description.', 'message': str(e)})
        }
    finally:
        connection.close()
    ...
    {
        'statusCode': 200,
        'body': f'Hello, {username} from CreateDescription!'
    }
